...
def normalize_evalex_config(config: dict[str, Any]) -> dict[str, Any]:
    """Normalize Evalex config: ensure cameraIds is list[str], ensure notification id, prune cameraLabels.
    
    Args:
        config: dict with notification_config fields
    ...
    # Ensure cameraIds is a list[str]
    ...
    # Ensure notification id exists
    if not normalized.get("id"):
        normalized["id"] = str(uuid.uuid4())
    
    # Prune cameraLabels to only keys in cameraIds
...
async def reconcile_evalex_config(
    config: dict[str, Any],
    logger_: logging.Logger = logger,
) -> tuple[dict[str, Any], dict[str, Any]]:
    """Reconcile Evalex config against live server: fetch settings, keep only live camera IDs.
    
    Args:
        config: normalized config dict with app, serverAddress, cameraIds
        logger_: logger instance
    
    Returns:
        (reconciled_config, metadata) where metadata has reconciliation status
    """
    metadata: dict[str, Any] = {"stale_removed": [], "unreachable": False}
    
    # Start with normalized config
    ...
    if not app or app not in ("vizmux", "piyoai", "vizrec"):
        logger_.warning(f"reconcile_evalex_config: invalid app {app!r}")
        return reconciled, metadata
    
    if not server_address:
        logger_.warning("reconcile_evalex_config: empty serverAddress")
        return reconciled, metadata
    ...
    try:
        ...
        logger_.warning(f"reconcile_evalex_config: failed to fetch settings from {server_address}: {e}")
        metadata["unreachable"] = True
        return reconciled, metadata
    
    # Extract live camera IDs based on app type
    ...
    try:
        ...
        logger_.warning(f"reconcile_evalex_config: error parsing settings for {app}: {e}")
        return reconciled, metadata
    
    # Rebind cameraIds: keep only those that exist on server
    ...
    if stale:
        ...
        logger_.info(f"reconcile_evalex_config: removed stale camera IDs: {stale}")
